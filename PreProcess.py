...
class SkimlitDataset(Dataset):
    ...
    def collate_fn(self, batch):
        ...
        line_nums = F.one_hot(torch.tensor(line_nums.astype(np.float32)).to(torch.int64), num_classes=51)

        # converting total lines into one-hot encoding
        total_lines = F.one_hot(torch.tensor(total_lines.astype(np.float32)).to(torch.int64), num_classes=51)

        # converting inputs to tensors
        pad_text_seq = torch.LongTensor(pad_text_seq.astype(np.int32))
        seq_lens = torch.LongTensor(seq_lens.astype(np.int32))
        line_nums = torch.tensor(line_nums, dtype=torch.float32)
        total_lines = torch.tensor(total_lines, dtype=torch.float32)

        return pad_text_seq, seq_lens, line_nums, total_lines

    def create_dataloader(self, batch_size, shuffle=False, drop_last=False):
        dataloader = DataLoader(dataset=self, batch_size=batch_size, collate_fn=self.collate_fn, shuffle=shuffle,
